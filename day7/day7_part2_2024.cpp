...
void day7_part2() {
    ...
    if (read_equations("input.txt", equations)) {
        ...
        std::cout << "The new total calibration result: " << result << '\n';
    }
}
...
bool eval_equation (std::vector<long long> eq) {
    while (eq.size() > 2) {
        ...
        if (eq[0] % calc_divisor(eq.back()) == eq.back()) { // concatenation is possible
            std::vector<long long> new_eq {eq};
            new_eq[0] /= calc_divisor(new_eq.back());
            new_eq.pop_back();
            if (eval_equation(new_eq)) {
                return true;
            }
...
int calc_divisor(long long last_num) {
    int exp{};
    while(last_num) {
        last_num /= 10;
        exp++;
